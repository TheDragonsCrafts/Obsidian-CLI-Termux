...
fn bench_search(c: &mut Criterion) {
    ...
    c.bench_function("contains_query_old", |b| b.iter(|| {
        black_box(contains_query_old(black_box(line), black_box(query_old), black_box(false)))
    }));

    c.bench_function("contains_query_new", |b| b.iter(|| {
        black_box(contains_query_new(black_box(line), black_box(query_new), black_box(false)))
    }));
}
